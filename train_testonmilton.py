...
if not sanity_check:    
    checkpoint_root = f"/data/stg60/Data_milton/new_model/version_{version_model}"
else:
...
root = Path(f"/data/stg60/milton_plotdata/version_{version_data}")
...
class TestResult(tf.keras.callbacks.Callback):    
    def on_epoch_end(self, epoch, logs=None):
        name = f"{rep_root}/version_{version_catalog}_report_sim.txt"
        ...
        name = f"{rep_root}/version_{version_catalog}_report_real.txt"
...
if mode == "train":
    ...
    with open(f"{rep_root}/test_report_kenneth_version_{version_catalog}.txt","w") as f:
        f.write(report)
