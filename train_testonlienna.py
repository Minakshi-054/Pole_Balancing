...
root = Path(f"/data/stg60/Data_lienna/version_{version_data}")
...
with open(f"{cat_root}/version_{version_catalog}.txt", "w") as f:
    ...
    with open("preparedata_leannasim.py", "r") as p:
        header_ = """
        ########################################################
        ###############       preparedata_leannasim.py       #############
        ########################################################
    ...
    with open("train_testonlienna.py", "r") as t:
        header_ = """
        ########################################################
        ###############       train_testonleanna.py       ###################
        ########################################################
...
class TestResult(tf.keras.callbacks.Callback):    
    def on_epoch_end(self, epoch, logs=None):
        name = f"{rep_root}/version_{version_catalog}_report_pred_leanna.txt"
        write_report(model = self.model, x = pred_examples, y = pred_labels, target_path = name, on = "pred")

        name = f"{rep_root}/version_{version_catalog}_report_par_leanna.txt"
        write_report(model = self.model, x = test_examples, y = test_labels, target_path = name, on = "par")

        name = f"{rep_root}/version_{version_catalog}_report_val_leanna.txt"
...
with open(f"{cat_root}/version_{version_catalog}.txt", "a") as f:
    ...
    par_path = f"{rep_root}/version_{version_catalog}_report_par_leanna.txt"
    pred_path = f"{rep_root}/version_{version_catalog}_report_pred_leanna.txt"
    val_path = f"{rep_root}/version_{version_catalog}_report_val_leanna.txt"
