...
data_root = Path("/data/stg60/processed_data_milton_updated/version_5/")
...
def getAllMats(path = "/data/stg60/new_miltondata/folder005", sample = None, filter_required = True, L = 128, timetopredict = 2, samplingrate = 100):
...
def prepareData(path, cutoff = 75, delay = 23, L = 128, timetopredict = 2, samplingrate = 100, fall_stride = 1, start_index = 1024, end_index = None, scale = True):
...
def getOneHugeArray(path_list, start_index_list = None, end_index_list = None):
    ...
    if start_index_list is None and end_index_list is None:    
        for path in tqdm(path_list):
            x,y,_ = prepareData(path, cutoff = 75, delay = 23, L = 128, timetopredict = 2, samplingrate = 100, fall_stride = 1)
            datax.append(x)
            datay.append(y)
    else:
        for path, start_index, end_index in tqdm(zip(path_list, start_index_list, end_index_list)):
            x,y,_ = prepareData(path, cutoff = 75, delay = 23, L = 128, timetopredict = 2, samplingrate = 100, fall_stride = 1, start_index = start_index, end_index = end_index)
            datax.append(x)
            datay.append(y)
