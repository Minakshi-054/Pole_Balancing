...
data_root = Path("/data/stg60/lienna_Data/version_1/")
...
def getAllMats(path = "/data/stg60/lienna_Data/folder001/", sample = None, filter_required = True, L = 128, timetopredict = 1.96, samplingrate = 100):
    maxsize = int(timetopredict*samplingrate) + L*4 + 1
    data = sorted(Path(path).glob("leanna*.mat"))
    if sample:
        data = random.sample(data, sample)
    if filter_required:
        data = list(filter(lambda x:loadmat(x)['phiv'].max()>18 and loadmat(x)['phiv'].size>maxsize, data))
    return data
