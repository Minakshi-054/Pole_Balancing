...
data_root = Path("/data/stg60/milton_plotdata/new/")
# data_kenneth_par = Path("/home/Students/stg60/Data/data/newmats/real/Prediction_project/Milton_for_parameter_est/FS_Milton0083.tsv")
# data_kenneth_pred = Path("/home/Students/stg60/Data/data/newmats/real/Prediction_project/Milton_for_prediction/")
...
def getAllMats(path = "/data/stg60/new_miltondata/newmilton_data/", sample = None, filter_required = True, L = 128, timetopredict = 1.96, samplingrate = 100):
    maxsize = int(timetopredict*samplingrate) + L*4 + 1
    data = sorted(Path(path).glob("milton*.mat"))
    if sample:
        data = random.sample(data, sample)
    if filter_required:
        data = list(filter(lambda x:loadmat(x)['phiv'].max()>3 and loadmat(x)['phiv'].size>maxsize, data))
    return data
