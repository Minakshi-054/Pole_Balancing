...
data_milton_par = Path("/data/stg60/new_realdata/Karl/test/")
data_milton_pred = Path("/data/stg60/new_realdata/Karl/refineTest/")
...
def getAllMats(path = "/data/stg60/new_karldata/folder001", sample = None, filter_required = True, L = 128, timetopredict = 1.96, samplingrate = 100):
...
def prepareData(path, cutoff = 32, delay = 23, L = 128, timetopredict = 1.96, samplingrate = 100, fall_stride = 1, start_index = 1024, end_index = None, scale = True):
...
def getOneHugeArray(path_list, start_index_list = None, end_index_list = None):
    ...
    if start_index_list is None and end_index_list is None:    
        for path in tqdm(path_list):
            x,y,_ = prepareData(path, cutoff = 32, delay = 23, L = 128, timetopredict = 1.96, samplingrate = 100, fall_stride = 1)
            datax.append(x)
            datay.append(y)
    else:
        for path, start_index, end_index in tqdm(zip(path_list, start_index_list, end_index_list)):
            x,y,_ = prepareData(path, cutoff = 32, delay = 23, L = 128, timetopredict = 1.96, samplingrate = 100, fall_stride = 1, start_index = start_index, end_index = end_index)
            datax.append(x)
            datay.append(y)
...
if __name__ == '__main__':
    ...
    #data1 = np.load(data_root/"train.npz")
    #data2 = np.load(data_root/"par.npz")
   ...
   # y = np.concatenate((y1, y2), axis=0)
    #np.savez(data_root/"combine.npz", x=x, y=y)
