...
if not sanity_check:    
    checkpoint_root = f"/data/stg60/cache3_trainsim/version_{version_model}"
else:
...
root = Path(f"/data/stg60/imbalanceddata/version_{version_data}")
...
with open(f"{cat_root}/version_{version_catalog}.txt", "w") as f:
    ...
    with open("preparedata_imbalanced.py", "r") as p:
        header_ = """
        ########################################################
        ###############       preparedata_imbalanced.py       #############
        ########################################################
    ...
    with open("train_imbalanced.py", "r") as t:
        header_ = """
        ########################################################
        ###############       train_imbalanced.py       ###################
        ########################################################
...
class TestResult(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        ...
        target_path = f"{rep_root}/version_{version_catalog}_report_test_milton.txt"
...
if mode == "train":
    ...
    print("Model Evaluation Started on par set")
    ...
    with open(f"{rep_root}/testing_par_report_version_{version_catalog}_imbalanced.txt","w") as f:
        f.write(report)
  
    print(report)

    print("Model Evaluation Started on pred set")
    ...
    with open(f"{rep_root}/testing_pred_report_version_{version_catalog}_imbalanced.txt","w") as f:
        f.write(report)
