...
if not sanity_check:    
    checkpoint_root = f"/data/stg60/trainon_sim_model/version_{version_model}"
else:
...
root = Path(f"/data/stg60/DATA/version_{version_data}")
...
with open(f"{cat_root}/version_{version_catalog}.txt", "w") as f:
    ...
    with open("train_regularizer.py", "r") as t:
        header_ = """
        ########################################################
        ###############       train_regularizer.py       ###################
        ########################################################
...
class TestResult(tf.keras.callbacks.Callback):    
    def on_epoch_end(self, epoch, logs=None):
        ...
        write_report(model = self.model, x = test_examples, y = test_labels, target_path = name, on = "sim")
...
def plot_predictions(y_real, y_pred, title=""):
    """
    Plots real outcome and prediction in a single graph
    Parameters
    ----------
    y_real : (n x 1) numpy array of outcomes
    y_pred : (n x 1) numpy array of probabillities
    title : optional graph title string
    """
    if len(y_real) != len(y_pred):
        raise Exception("Size of y_real != Size of y_pred")
    x_arange = np.arange(1, len(y_real) + 1)
    y_real = y_real.flatten()
    y_pred = y_pred.flatten()

    fig = go.Figure()
    fig.add_scatter(x=x_arange, y=y_real, name="Actual", fill="tozeroy")
    fig.add_scatter(x=x_arange, y=y_pred, name="Predicted")
    fig.update_yaxes(matches="y", fixedrange=True)
    fig.update_layout(title=title)
    fig.show() 
   


def plot_pr_curve(y_real, y_pred, title="PR Curve"):
    """
    Plots PR curve
    Parameters
    ----------
    ...
    title : optional graph title string
    """
    y_real = y_real.flatten().astype(int)
    y_pred = y_pred.flatten()
...
if mode == "train":
    ...
    print(report)
    plot_predictions(test_labels, y_out)
    plot_pr_curve(test_labels, y_out, title="PR Curve")
...
with open(f"{cat_root}/version_{version_catalog}.txt", "a") as f:
   ...
   # par_path = f"{rep_root}/version_{version_catalog}_report_par_milton.txt"
    ...
    val_path = f"{rep_root}/version_{version_catalog}_report_test_milton.txt"
