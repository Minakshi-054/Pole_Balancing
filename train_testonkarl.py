...
root = Path(f"/data/stg60/Data_karl/version_{version_data}")
...
with open(f"{cat_root}/version_{version_catalog}.txt", "w") as f:
    ...
    with open("preparedata_karl.py", "r") as p:
        header_ = """
        ########################################################
        ###############       preparedata_karl.py       #############
        ########################################################
    ...
    with open("train_testonkarl.py", "r") as t:
        header_ = """
        ########################################################
        ###############      train_testonkarl.py      ###################
        ########################################################
...
class TestResult(tf.keras.callbacks.Callback):    
    def on_epoch_end(self, epoch, logs=None):
        name = f"{rep_root}/version_{version_catalog}_report_test_kenneth.txt"
        ...
        name = f"{rep_root}/version_{version_catalog}_report_val_kenneth.txt"
...
if mode == "train":
    ...
    with open(f"{rep_root}/test_report_karl_version_{version_catalog}.txt","w") as f:
        f.write(report)
